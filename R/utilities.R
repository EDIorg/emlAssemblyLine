...
set_methods_md <- function(methods_file) {
  ...
  i_eq <- as.data.frame(stringr::str_locate_all(md, "\\$\\$.*\\$\\$")) # Equation indices (start and end)
  res <- list()
  ...
  # FIXME An edge case that is not yet supported. Warning is the best we can do right now.
  eol_regex <- paste(c("\\n", "\\r\\n", "\\r"), collapse = "|")
  is_only_eol <- stringr::str_detect(res, eol_regex) & nchar(res) <=2
  if (any(is_only_eol)) {
    warning("LaTeX equations cannot be separated by only empty lines in 'methods.md'.")
  }
