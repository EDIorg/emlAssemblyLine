...
template_other_entity <- function(
  path, 
  data.path = path, 
  other.entity = NULL, 
  write.file = TRUE,
  ...
  if (isTRUE(write.file)) {
    ...
    # FIXME: Only non-existing other entities should be added.
    ...
    for (i in 1:length(other.entity)) {
      if (!(other.entity[i] %in% df$value)) {
        ...
        df[(nrow(df)+1):(nrow(df)+4), ] <- data.frame(
          id = rep(id, 4),
          element = c("otherEntity", "otherEntityName", "otherEntityDescription", "url"),
